...
@section('content_admin')
            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page">Publikasi Activities Katar Anyelir</li>
                    </ul>
                </div>
                <div class="col-md-12">
                    <div class="page-header-judul">
                        <h2 class="mb-0">Publikasi Activities Katar Anyelir</h2>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="dt-responsive table-responsive">
                                ...
                                <div class="py-3">
                                    <button type="button" class="btn btn-primary me-3" data-bs-toggle="modal"
                                        data-bs-target="#AddactivitykatarModal">
                                        Tambah Publikasi
                                    </button>
                                </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-primary me-3"
                                                        ...
                                                        data-bs-target="#UpdateactivitykatarModal-{{ $item->id }}">
                                                        Update
                                                    </button>
                                                    <button type="button" class="btn btn-danger me-3"
                                                        data-bs-toggle="modal"
                                                        data-bs-target="#DeleteactivitykatarModal-{{ $item->id }}">
                                                        Delete
                                                    </button>
                                                    <button type="button" class="btn btn-info me-3" data-bs-toggle="modal"
                                                        data-bs-target="#DetailactivitykatarModal-{{ $item->id }}">
                                                        Detail
                                                    </button>
                                                </td>
                                            </tr>
    ...
    <!-- Modal Tambah activitykatar -->
    <div id="AddactivitykatarModal" class="modal fade" tabindex="-1" role="dialog"
        aria-labelledby="AddactivitykatarModaljudul" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-judul">Form Tambah Publikasi Katar RW 12</h5>
                        ...
                        @foreach ($fotoList as $foto)
                            <div class="col-md-3 col-sm-4 mb-3 foto-item"
                                ...
                                <label class="d-block border rounded p-2 text-center">
                                    <input type="checkbox" name="fotokatar[]" value="{{ $foto->id }}"
                                        class="form-check-input mb-2">
                                    <img src="{{ asset('storage/' . $foto->fotokatar) }}" class="img-fluid rounded mb-1"
                                        alt="Foto">
    ...
    <div class="modal fade" id="AddfotoModal" tabindex="-1" aria-labelledby="AddFotoModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('activitykatar.store_kft') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header bg-primary text-white">
                    ...
                    <div class="modal-body">
                        <div class="form-group mb-3">
                            <label for="fotokatar">Pilih Foto</label>
    ...
    <!-- Modal Update activitykatar -->
    @foreach ($activities as $item)
        <div id="UpdateactivitykatarModal-{{ $item->id }}" class="modal fade" tabindex="-1" role="dialog"
            aria-labelledby="UpdateactivitykatarModalTitle-{{ $item->id }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title" id="UpdateactivitykatarModalTitle-{{ $item->id }}">
                    ...
                    <div class="modal-body">
                        <div class="card">
                            <form action="{{ route('activitykatar.update_katar', $item->id) }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div id="fotoList-{{ $item->id }}" class="d-flex flex-column gap-3">
                                    @foreach ($item->dokumentasi as $foto)
                                        <input type="hidden" name="fotokatar[]" value="{{ $foto->id }}">
                                    @endforeach
                                </div>
                        ...
                        <div class="row" id="fotoGallery-{{ $item->id }}">
                            ...
                            @foreach ($fotoList as $foto)
                                <div class="col-md-3 col-sm-4 mb-3 foto-item"
                                    data-caption="{{ strtolower($foto->caption ?? '') }}">
                                    <label class="d-block border rounded p-2 text-center">
                                        <input type="checkbox" name="fotokatar-{{ $item->id }}[]"
    ...
    @foreach ($activities as $item)
        <div id="DeleteactivitykatarModal-{{ $item->id }}" class="modal fade" tabindex="-1" role="dialog"
            aria-labelledby="DeleteactivitykatarModalTitle-{{ $item->id }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('activitykatar.destroy_katar', $item->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <div class="p-3 text-center">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    @if ($item->dokumentasi->isNotEmpty())
                                        <img src="{{ asset('storage/' . $item->dokumentasi->first()->fotokatar) }}"
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-info text-white">
                        <h5 class="modal-title" id="DetailactivitykatarModalLabel-{{ $item->id }}">
    ...
    <script>
        ...
        window.clearFotoSelectionCreate = function() {
            ...
            const checkedInputs = document.querySelectorAll('#FotoPickerModal input[name="fotokatar[]"]:checked');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ==================================================
            const createModal = document.getElementById('AddactivitykatarModal');
            if (createModal) {
                ...
                if (createSelectFotoBtn) {
                    createSelectFotoBtn.addEventListener('click', function() {
                        ...
                        const checkedInputs = document.querySelectorAll(
                            '#FotoPickerModal input[name="fotokatar[]"]:checked');
                        checkedInputs.forEach(input => {
                            const hiddenInput = document.createElement('input');
                            hiddenInput.type = 'hidden';
                            hiddenInput.name = 'fotokatar[]';
            ...
            // ==================================================
            document.querySelectorAll('.modal[id^="UpdateactivitykatarModal-"]').forEach(modal => {
